...
import { getCommentCapabilities, canEditContent, canDeleteContent } from "@orbit/shared";
import { LetterThreadPanel } from "../components/LetterThreadPanel";
import { NotFoundState } from "../components/NotFoundState";
import { useAiArticleMeta } from "../contexts/aiArticleContext";
...
export function ArticleView() {
  ...
  return (
    <Container size="standard" className="orbit-article-layout flex gap-4 xl:gap-5">
      <div className="orbit-article-main min-w-0 flex-1 flex flex-col">
        {/* 1. 顶部纯净操作条 (Top Navigation & Actions Bar) */}
        <header className="orbit-article-nav-bar">
          <button
            type="button"
          ...
          >
            <ArrowLeftIcon />
            <span>返回{TYPE_LABEL[type || ""] ? TYPE_LABEL[type || ""] : "列表"}</span>
          </button>

          <div className="orbit-article-nav-actions">
            {entry.type === "letter" && (
              <Button
                variant="primary"
                size="sm"
                onClick={() =>
                  navigate(
                    `/letter/new?replyTo=${encodeURIComponent(entry.parentId ?? entry.id)}`
                  )
                }
              >
                写回信
            ...
            {canEditEntry && (
              <Button
                variant="secondary"
                size="sm"
                onClick={() => navigate(`/${type}/${entry.id}/edit`)}
              >
                编辑
              </Button>
            )}
        ...
        {/* 2. 纸质卡片阅读主体 (The Editorial Article Sheet) */}
        <article className="orbit-article-sheet">
          <header className="orbit-article-header">
            <ArticleMetadata
              title={entry.title}
              type={entry.type}
              author={entry.author}
        ...
        {/* 3. 评论区 (Comments Section) */}
        {capabilities.bottom && (
          <section className="orbit-article-comments-wrap">
            <CommentSection
