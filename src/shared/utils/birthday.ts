...
export function parseRemindCalendar(
  solar: SolarBirthday | null,
  lunar: LunarBirthday | null,
  requested?: unknown
):
  ...
  return { ok: true, value: requested as BirthdayCalendar };
}
...
export function parseBirthdayInput(
  ...
  const remind = parseRemindCalendar(solar, lunar, body.remindCalendar);
...
export function birthdayFromRow(row: {
  ...
  const remind = parseRemindCalendar(
    solar,
    lunar,
