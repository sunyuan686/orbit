...
| 用户消息署名 | `role=user` 的消息记录 `author`（User A / User B），共享线程中区分谁说的 |
...
| `author` | text | 所有者规范名（User A / User B） |
...
| `author` | text? | 仅 `role=user` 时：User A / User B |
...
```typescript
{
  ...
  ownerAuthor: string;        // User A | User B
  updatedAt: number;
...
#### Tool 2: `search_entries`

- 数据范围 = 整个空间（双人共享），不做 author 过滤（除非用户明确要求「只看 User A 写的」——由模型在 search 后过滤）
- 支持动态按 `type`（`diary` / `timeline` / `memo` / `letter` / `message`）过滤
- 执行 `db.select().from(entry)` 带 FTS match；如果 query 为空则返回最新 N 条
...
- [x] ARCHITECTURE 表结构补充（`ai_conversation` / `ai_message`，见 ARCHITECTURE.md:62）
...
| 默认私密 | User B 看不到 User A 的未共享会话 |
| 打开共享 | User B 列表出现「User A 共享」；可读可续聊 |
| 关闭共享 | 对方随后访问该会话 404 |
| 非所有者删/关共享 | 403 |
