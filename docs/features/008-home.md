...
自上而下；「最近照片」「恋爱记忆」在无数据时隐藏：

| 区块 | 作用 |
|------|------|
| Hero | 双方爱称、在一起天数（或 slogan）、纪念日文案；链到空间档案 |
| 记录节奏 | 连续天数、活跃天摘要、12 周迷你热力图；链到 `/activity` |
| 恋爱记忆 | 瞬间数、里程碑数、最近点亮文案、最近一篇；链到 `/memories` |
| 快速记录 | 写日记 / 写信 / 留言 |
| 探索 | 导航卡：日记、时间线、留言板、信箱、备忘录、相册、记忆 |
| 最近照片 | 相册最新 8 张缩略图 |
...
| `GET /api/memories/summary` | 恋爱记忆摘要 |
...
- 恋爱记忆：一行 meta + 可选最近瞬间行；不抢 Hero  
- 探索卡：图标 + 标题 + 一行说明，响应式网格  
- 照片：正方形缩略图网格，点击进入相册页  
- 动态列表：复用列表页信息密度（类型标签、标题、作者、日期）
...
| `web/src/components/OrbitIcons.tsx` | `HomeIcon`、`MemoriesIcon` |
...
空间封面图进 Hero、内容统计小卡（篇数 / 照片数）、下一纪念日倒计时、通知摘要。记忆摘要已落地。排期以 ROADMAP 为准。
